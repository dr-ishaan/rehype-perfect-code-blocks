...
async function transformPre(pre, opts) {
    ...
    if (opts.classActiveCode !== false && preLevelClasses.size > 0) {
        const existingCodeClasses = codeDataProps.className ?? [];
    ...
    if (opts.languageIcons && resolved.language) {
...
const _watchCache = new Map();
function hashBlock(lang, code, theme, meta) {
    let h = 0;
