 ...
 */
export function resolveMathOptions(math) {
    return {
 ...
 */
export function isMathLanguage(lang) {
 ...
 */
export async function renderMath(latex, displayMode, options) {
 ...
 */
export async function processMathInText(text, options) {
 ...
 */
async function processInlineMath(text, options) {
    const segments = [];
 ...
 */
export function getKatexCssPath() {
 ...
 */
export async function tryReadKatexCss() {
    try {
        const katex = await getKatex();
        if (!katex)
            return null;
